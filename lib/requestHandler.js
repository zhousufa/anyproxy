  ...
  return function (req, userRes) {
    ...
    */
    ...
    if (
      ...
      reqHandlerCtx.connects[req.socket.localPort]
    ) {
      const xForward = reqHandlerCtx.connects[req.socket.localPort];
...
function getConnectReqHandler(userRule, recorder, httpsServerMgr) {
  ...
  reqHandlerCtx.connects = new WeakMap();
      ...
      .then(serverInfo => {
        ...
        return new Promise((resolve, reject) => {
          const conn = net.connect(serverInfo.port, serverInfo.host, () => {
            ...
            reqHandlerCtx.connects.set(conn.localPort, xForward);
