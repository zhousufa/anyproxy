        ...
        const finishCollecting = () => {
          new Promise((fulfill, rejectParsing) => {
            if (resDataStream) {
              ...
              resHeader['X-Forwarded-For'] = options.headers['X-Forwarded-For'];
              /**
...
function getUserReqHandler(userRule, recorder) {
  ...
  return function (req, userRes) {
    ...
    */
    ...
    if (
      reqHandlerCtx.connects &&
      reqHandlerCtx.connects.get(req.socket.localPort)
    ) {
      const xForward = reqHandlerCtx.connects.get(req.socket.localPort);
      req.headers['X-Forwarded-For'] = xForward;
    }
...
function getConnectReqHandler(userRule, recorder, httpsServerMgr) {
  ...
  reqHandlerCtx.connects = new LRU(500);
  return function (req, cltSocket, head) {
    ...
    const xForward = req.socket.remoteAddress.split(':').pop();
      ...
      .then(serverInfo => {
        ...
        return new Promise((resolve, reject) => {
          const conn = net.connect(serverInfo.port, serverInfo.host, () => {
            ...
            if (reqHandlerCtx.connects.get(conn.remotePort) !== xForward) {
              reqHandlerCtx.connects.set(conn.remotePort, xForward);
            }
